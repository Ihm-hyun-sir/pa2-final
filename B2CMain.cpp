...
public:
 ...
 //  map<string,string> symbol_table;
  ...
  any visitIfstmt(BParser::IfstmtContext *ctx) override {
	...
	if(expr_type != "bool")
	{
		throw runtime_error("if statement conditions must be a boolean type");
	}
   ...
   any visitWhilestmt(BParser::WhilestmtContext *ctx) override {
        ...
        if(expr_type != "bool")
        {
                throw runtime_error("while statement conditions must be a boolean type");
        }
        visit(ctx->statement());
        return nullptr;
  }
